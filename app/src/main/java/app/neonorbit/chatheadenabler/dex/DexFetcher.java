...
public class DexFetcher {
    ...
    static {
        ...
        ReferenceFilter filter = ReferenceFilter.fieldsContain(Constants.REFERENCE_FIELD).and(
    ...
    private MethodData fastFetch(@NonNull Class<?> target) {
        ...
        return dexplore.findMethod(
                DEX_FILTER.toBuilder().allowPreferredDexOnly(true).build(),
                CLASS_FILTER.toBuilder().setClasses(clazz).build(), METHOD_FILTER
        );
    }
